...
def _manage_users(request, course_key):
    ...
    if STUDIO_VIEW_USERS not in user_perms:
    ...
    return render_to_response('manage_users.html', {
        ...
        'allow_actions': STUDIO_EDIT_ROLES in user_perms,
    })
...
def _course_team_user(request, course_key, email):
    ...
    if (STUDIO_VIEW_USERS in requester_perms) or (email == request.user.email):
    ...
    if not (STUDIO_EDIT_ROLES in requester_perms or (user.id == request.user.id)):
        return permissions_error_response
    ...
    for role_type in role_hierarchy:
        ...
        if role_type.ROLE == new_role:
            if STUDIO_EDIT_ROLES in requester_perms or (user.id == request.user.id and old_roles):
